use crate::{auth::Doggercom, debug::DisplayBytes, ArgMode};
use anyhow::Result;
use bstr::ByteSlice;
use digest::Digest;
use md4::Md4;
...
impl Doggercom {
    pub fn keepalive_1(&self, auth_information: &[u8; 16]) -> Result<()> {
        ...
        if config.keepalive1_mod {
            ...
            self.send(&keepalive_1_packet1)?;
            debug!(
                "[Keepalive1_packet1 sent] {}",
                DisplayBytes(keepalive_1_packet1)
            );
            loop {
                self.recv(&mut recv_packet1)?;
                debug!(
    ...
    pub fn keepalive_2_packetbuilder(
        ...
        if filepacket {
            ...
            keepalive_2_packet[6..8].copy_from_slice(&self.config.keep_alive_version);
        }
